...
class Submigrate
{
	...
	/**
	 * @var	array	list of subpackages to migrate
	 */
	protected static $subpackages = array();
	...
	/**
	 * @var	int	number of subpackages migrated
	 */
	protected static $subpackage_count = 0;
	...
	public function __call($name, $args)
	{
		...
		// run migrations on all specified subpackages
		foreach (static::$subpackages as $subpackage)
		{
			static::$name($subpackage, 'subpackage');
		}
	}
	...
	protected static function _run($name, $type)
	{
		...
		elseif ($version !== '')
		{
			...
			$migrations = \Subpackage\Submigrate::version($version, $name, $type);
		}

		// migrate to the latest version
		else
		{
			$migrations = \Subpackage\Submigrate::latest($name, $type);
		}
	...
	protected static function _current($name, $type)
	{
		...
		$migrations = \Subpackage\Submigrate::current($name, $type);
	...
	protected static function _up($name, $type)
	{
		...
		$migrations = \Subpackage\Submigrate::up($version, $name, $type);
	...
	protected static function _down($name, $type)
	{
		...
		$migrations = \Subpackage\Submigrate::down($version, $name, $type);
